...
public interface OrderMapper {
    ...
    @Select("select * from 销售订单 where  状态 !='删除' and 业务经理 like CONCAT('%',#{uname},'%')  order by id DESC limit 10")
    @Results({
    ...
    @Select("select * from 销售订单 where 状态 !='删除' and 业务经理 like CONCAT('%',#{uname},'%') order by id DESC limit #{limit},5")
    @Results({
            @Result(property = "id", column = "Id"),
            @Result(property = "orderTime", column = "订单日期"),
            @Result(property = "orderNumber", column = "订单号"),
    ...
    @Select("select * from 销售订单 where  状态 !='删除' and 业务经理 like CONCAT('%',#{uname},'%') and 客户名称=#{customerName} order by id DESC limit 10")
    @Results({
            @Result(property = "id", column = "Id"),
            @Result(property = "orderTime", column = "订单日期"),
            @Result(property = "orderNumber", column = "订单号"),
