...
<div class="mui-content">
    ...
    <div class="mui-content-padded" style="margin-top: 20px;">
        <button id='id_btnSave' type="button" class="mui-btn mui-btn-success" style="width: 100%;">修改申请单</button>
    </div>
</div>
</body>
...
<script>
    function dynamicDiv(apply) {
        $("#changed").empty();
        if ("退款" == apply.type) {
            html = '<div class="mui-input-row"><label>退款金额</label> <input type="text" placeholder="" id="refundAmount" value="' +apply.refundAmount+ '"></div>' +
                '<div class="mui-input-row"><label>退款账户</label> <input type="text" placeholder="" id="refundAccount" value="' +apply.refundAccount+ '"></div>'
            $("#changed").append(html);
        }
        if ("调账" == apply.type) {
            html = '<div class="mui-input-row"><label>调账金额</label> <input type="text" placeholder="" id="adjustmentAmount" value="' +apply.adjustmentAmount+ '"></div>' +
                '<div class="mui-input-row"><label>调后余额</label> <input type="text" placeholder="" id="balance" value="' +apply.balance+ '"></div>'
            $("#changed").append(html);
        }
        if ("客情" == apply.type) {
            html = '<div class="mui-input-row"><label>费用金额</label> <input type="text" placeholder="" id="costs" value="' +apply.costs+ '"></div>'
            $("#changed").append(html);
        }
        if ("调价" == apply.type) {
            html = '<div class="mui-input-row"><label>原单价</label> <input type="text" placeholder="" id="originalPrice" value="' +apply.originalPrice+ '"></div>' +
                '<div class="mui-input-row"><label>现单价</label> <input type="text" placeholder="" id="presentPrice" value="' +apply.presentPrice+ '"></div>'
            $("#changed").append(html);
        }
        if ("赠品" == apply.type) {
            html = '<div class="mui-input-row"><label>赠品赠送方式</label> <input type="text" placeholder="" id="givingType" value="' +apply.givingType+ '"></div>' +
                '<div class="mui-input-row"><label>折合单价</label> <input type="text" placeholder="" id="conversionPrice" value="' +apply.conversionPrice+ '"></div>'
            $("#changed").append(html);
        }
    }
...
<script>
    ...
    btnSave.addEventListener("tap", function () {
        ...
        var obj = {"id":$("#id").val(),"customerName": $("#customerName").val(),"customerNo":$("#customerNo").val(),"type":$("#typePicker").html(),"content":$("#content").val()};
